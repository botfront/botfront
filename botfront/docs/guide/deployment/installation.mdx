...
| `APPLICATION_LOG_LEVEL`         | From witch level the log are logged                                                                                                                                        | critical, error, warn, info, debug | info                 |
| `APPLICATION_LOGGER_NAME`       | The name of the application logger on stackdriver                                                                                                                          | Any string                         | botfront_log_app     |
| `MAX_LOGGED_ARG_LENGHT`         | Maximum lenght of a stringified callingArgs entry, for readability/data usage concerns. Used only when logging at info level and when APPLICATION_LOG_LEVEL is set to info | Any positive integer               | 1000                 |
| `AUDIT_LOG_TRANSPORT`           | Where the audit log are written                                                                                                                                            | console, stackdriver               | console              |
| `AUDIT_LOGGER_NAME`             | The name of the  audit logger on stackdriver                                                                                                                               | Any string                         | botfront_log_audit   |
| `APPLICATION_API_LOGGER_NAME`   | The name of the api application logger on stackdriver                                                                                                                      | Any string                         | botfront-api_log_app |
