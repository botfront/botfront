...
# Proactive conversations <span class="tag is-warning">Premium Feature</span>
...
- **Choose a limit:** The story will be triggered each time the condition set in this this trigger are met, but only for the number of times set.
...
Choose the minimum ammount of time between two triggers of the same story, in minutes. In the above example, the interval is set to 24 hours, so the trigger will only be able to trigger once every day.
