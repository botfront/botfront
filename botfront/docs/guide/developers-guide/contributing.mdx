...
Under the hood, we use winston logger. Creating an instance is done throught 3 helper functions (see [Creating a logger](#createlogger)).
We use a logger instance per file and per method.
Once you have acces logger instance in the methods you working in you can refer to the winston docs if you need more details on available methods.

The logging is only done on the server-side part of the application.
using our logger you can log at 4 criticality levels :
- error
- warn
- info
- debug

You can also add metadata to be logged statement to provide more details. However, we limit the metadata you can log to: 
...
// Synonym with te previous statement
...
Log statement are formated following this format, empty fields do not appear
```
<timestamp> [<level>] : <message> user: <userId> <file> - <method> with <args> url: <url> <status> data: <data>  error: <error> 
```
...
## Creating a logger <a name="createlogger"></a>
...
```javascript
 import {
    ...
    } from 'botfront/server/logger.js';
```
Don't forget to change the path to the correct one for the file you wish to log from.
...
This method will create a logger for the whole file, it just create a logger with the file metadata prefilled
