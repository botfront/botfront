...
if (Meteor.isServer) {
    ...
    describe('training data has fuzzy gazette', function() {
        it('export should have gazette when enabled', function() {
            const output = getTrainingDataInRasaFormat(
                model2,
                false,
                [],
                false,
    ...
    // commented out those tests because the method now relies on connection to mongo,
    // which we can not provide in that context
    describe('config yaml', function() {
        ...
        //     expect(yaml.load(config).pipeline[2].entities).to.deep.equal([
        //         { name: 'test_name', mode: 'ratio', min_score: 80 },
        //     ]);
        // });
